...
/// `.auto` を使用すると、ソースウインドウとターゲット矩形の相対位置からライブラリが方向を自動判定する。
/// 具体的な方向 (`.bottom`, `.top`, `.left`, `.right`) を指定することで、ワープの辺を明示的に制御することもできる。
public enum GenieDirection {
	...
	/// ソースとターゲットの矩形の中心間の水平・垂直距離を比較し、支配的な軸に基づいて方向を決定する。
